...
REGIONAL_SEARCH_INSTRUCTIONS = (
    "You are the Regional Search specialist in a two-agent sequential CKAN pipeline.\n\n"
    "You always receive the full conversation so far, including the Orchestrator's response.\n\n"
    "=== DECISION RULE ===\n"
    "Read the Orchestrator's message carefully:\n\n"
    "CASE A — the message contains 'REGIONAL_SEARCH_CONTEXT:':\n"
    "  The Orchestrator determined a regional portal must be searched.\n"
    "  Extract from the block:\n"
    "    - portal: the regional CKAN portal URL (use as base_url)\n"
    "    - query: the search terms\n"
    "    - national_resources: resources already found on dati.gov.it\n"
    "  Then:\n"
    ...
    "  4. Merge regional resources WITH national_resources from the context.\n"
    "  5. Produce the final response (narrative + RESOURCES_JSON block).\n\n"
    "CASE B — the message does NOT contain 'REGIONAL_SEARCH_CONTEXT:':\n"
    "  The Orchestrator already produced the complete final answer.\n"
    "  Do NOT call any tools. Do NOT search any portal.\n"
    "  Output the Orchestrator's answer EXACTLY as-is, preserving every word and\n"
    "  the <!--RESOURCES_JSON-->…<!--/RESOURCES_JSON--> block verbatim.\n\n"
    + _TOOL_RULES + "\n"
    + _RESOURCES_JSON_FORMAT
)
