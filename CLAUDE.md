...
`opendata-ai` is a polyglot mono-repo that fans a user query across **up to four open-data sources** (CKAN portals + three SDMX-based statistical providers: ISTAT, Eurostat, OECD), synthesises a single answer, and renders GeoJSON results on OSM maps. Six Python services + one Next.js UI:
...
- `osm-mcp/` — FastMCP server (renderer-only here) that turns GeoJSON into self-contained Leaflet+OSM HTML maps. Default container port `8080`, exposed on host `8085`. The orchestrator calls its `compose_map_from_resources` tool over MCP streamable-HTTP (no LLM, no osm-agent) to attach a `preview_html` map to each GeoJSON resource. Tools also include geocoding/POI/routing, unused for now.
- `opendata-orchestrator/` — **Multi-agent orchestrator** built with `agent_framework.orchestrations.ConcurrentBuilder`. Fans the query out in parallel to up to 4 specialists (CKAN + ISTAT + Eurostat + OECD; the last two are opt-in via `ENABLE_EUROSTAT` / `ENABLE_OECD`). The three SDMX specialists all dial **the same** `istat-mcp-server` — they differ only by the `agency` + `base_url` baked into their instructions. A tool-less `synth` agent then merges their narratives. After merging, GeoJSON resources are sent to `osm-mcp` to attach a `preview_html` map (`ENABLE_OSM_MAPS`, `OSM_MCP_URL`). Returns the canonical `{text, resources[]}` shape with `source`, optional `description`, and `preview_html` per resource. CLI `opendata-agent` + FastAPI `opendata-orchestrator-api` on port `8000`. **This is the entry the UI talks to.**
- `opendata-ai-ui/` — Next.js 15 chat demo (port `3000`). Proxies `/api/chat` to the orchestrator. Two pages: `/` (tabular/JSON/charts chat — ResourceCard shows source badge, Tabella|Grafico toggle, DataDescription caption, and an inline OSM map iframe when a resource has `preview_html`); `/mappa` (a geo-focused chat — a persistent client-side Leaflet map that accumulates GeoJSON layers from each turn with a layer toggle).
