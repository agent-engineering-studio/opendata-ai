"""Entry point for the OpenDataSoft MCP server.
...
def build_server() -> FastMCP:
    mcp = FastMCP(
        name="ods-mcp-server",
        instructions=(
            "Tools to query any OpenDataSoft open data portal via the Explore API v2.1. "
            "Each tool accepts an optional base_url; when omitted, the server's default "
            "ODS_DEFAULT_BASE_URL is used. Filters use ODSQL."
        ),
        host=HOST,
        port=PORT,
...
def main() -> None:
    ...
    if TRANSPORT == "stdio":
        log.info("Starting OpenDataSoft MCP server over stdio")
        mcp.run(transport="stdio")
    elif TRANSPORT in ("http", "streamable-http", "streamable_http"):
        log.info("Starting OpenDataSoft MCP server on http://%s:%s%s", HOST, PORT, MCP_PATH)
        mcp.run(transport="streamable-http")
    elif TRANSPORT == "sse":
        log.info("Starting OpenDataSoft MCP server (SSE) on http://%s:%s", HOST, PORT)
