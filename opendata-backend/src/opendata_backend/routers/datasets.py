...
async def search_stream(
    ...
    async def _one_event(ev):
        ...
        if ev.get("event") == "result":
            raw = ev.get("text") or ""
            text, resources = parse_agent_reply(raw)
            # The Ollama CKAN specialist can't be trusted to surface real data —
            # on "piste ciclabili Bologna" it never called the search tool and
            # hallucinated a fake resource URL (404). So ALWAYS run a deterministic
            # CKAN search and merge the verified resources; the agent keeps the
            # narrative, the direct search guarantees the datasets/geometry.
            extra = await ckan_geo_fallback(
                req.query,
