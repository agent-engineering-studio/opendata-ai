...
PROGRAMMA_INSTRUCTIONS = (
    ...
    "CITAZIONE OBBLIGATORIA: una voce SWOT SENZA un'evidenza con URL del bundle "
    "viene SCARTATA dal validatore — non sprecarla. Per ogni fatto che riguarda "
    "una lente (posti letto, fermate bus, imprese, indici demografici…) l'URL è "
    "GIÀ nel bundle (sezioni LENTE/RISORSE CITABILI): copialo VERBATIM nelle "
    ...
    "- `fattibilita`: lo spend ratio OpenCoesione è SPECIFICO PER TEMA. Riportalo "
    "in `spend_ratio_storico` SOLO se è dello STESSO tema della proposta — lo "
    "spend ratio 'inclusione sociale' vale per una proposta welfare/sociale, NON "
    "per una turistica, commerciale o di mobilità. Quando il tema combacia, "
    "motivalo citando il tema; per le proposte di altro tema lascia "
    "`spend_ratio_storico: null` e fonda la fattibilità su finanziamento "
    "disponibile e vincoli. Una proposta senza evidenza di finanziamento ha "
    "`finanziamento: null` e livello `da_verificare`.\n"
...
IDEE_INSTRUCTIONS = (
    ...
    "  - gap_comparativo — 'i comuni simili l'hanno fatto, qui no': progetti "
    "finanziati da comuni comparabili assenti nel comune in esame. I progetti "
    "peer REALI sono nel bundle, sezione COMPARABILI (uno per riga, con titolo, "
    "CLP, importo e URL /progetti/): le evidenze DEVONO includere l'URL del "
    "PROGETTO SPECIFICO da quella sezione e il `dettaglio` deve riportarne "
    "titolo, importo ed esito. Se la sezione COMPARABILI è assente o nessun "
    "progetto è pertinente, NON usare questo generatore e NON inventare un "
    "comparabile.\n"
    ...
    "specifico se elencato (es. 'valorizzare il Castello X') usando il NOME e il TIPO "
    "ESATTI dati da OSM: NON inferire la natura dell'asset oltre la sua etichetta "
    "(non chiamarlo 'sito archeologico', 'museo' o simili se il dato non lo dice — "
    "es. una ex base militare resta tale, non un sito archeologico). Incrocia asset, posti "
    ...
    "ordine (→ pendolarismo) — MA dichiarala SOLO se il conteggio di QUELL'ordine nel "
    "blocco è 0; se «sec. II grado» (o un altro ordine) ha N>0 plessi, è FALSO dire che "
    "è assente o proporre di «crearlo»: con plessi presenti la leva è qualità/indirizzi/"
    "contrasto dispersione, non una scuola mancante. Usa i conteggi e le ETICHETTE ESATTE "
    "del blocco («sec. II grado» = scuole superiori, NON «secondo anno»). Inoltre pochi "
    "plessi/alunni o calo iscritti (→ accorpamenti), MA "
    "anche BASSA quota di laureati/diplomati, analfabetismo o alta uscita precoce "
    "(capitale umano debole). ANCORA = i numeri del blocco: citane SEMPRE il source_url "
    "(MIUR per l'offerta, 8milaCensus per il grado); incrocia con la popolazione. Proponi "
    ...
    "ISPRA IdroGEO del blocco 'LENTE AMBIENTE' (% territorio e popolazione esposta): "
    "citane SEMPRE il source_url. Tratta la pericolosità come VINCOLO di pianificazione "
    "(localizza altrove o prevedi mitigazione/messa in sicurezza) e proponi interventi "
    "coerenti coi numeri; se le quote sono ~0, l'assenza di vincolo è essa stessa "
    "un'evidenza (es. 'territorio a basso rischio → idoneo a nuove funzioni'). NIENTE "
    "precedente web. Se NON ci sono indicatori ISPRA nel bundle, NON forzare l'idea: saltala.\n"
    "  - sanita — 'dove agire sull'accessibilità ai servizi sanitari': individua la "
    "dotazione del comune — FARMACIE (presidio più capillare), OSPEDALI e STRUTTURE "
    "TERRITORIALI (ambulatori/studi medici) — e la sua adeguatezza rispetto alla "
    "popolazione. ANCORA = i conteggi del blocco 'LENTE SANITÀ' (farmacie Min. Salute "
    "+ presìdi OpenStreetMap): citane SEMPRE il source_url; incrocia con la popolazione "
    "(farmacie/presìdi per abitante). Leggi i gap: assenza di ospedale nel comune → "
    "mobilità sanitaria (usa la DISTANZA/TEMPO dall'ospedale più vicino, se nel blocco, "
    ...
    "nel bundle con il suo URL `/progetti/…` (mettilo nelle `evidenze`). Se nel "
    "bundle NON c'è un progetto peer, è VIETATO inventare nome, CLP, importo o "
    "percentuali di esito: parla in modo generico e dichiaratamente ipotetico "
    "('interventi analoghi avviati da comuni simili', senza cifre o codici "
    "inventati); (4) l'ordine di grandezza del costo: dedotto SOLO dagli importi "
    "di progetti REALMENTE presenti nel bundle (es. '~2-3 M€ sul modello del "
    "progetto X, CLP … '); se nel bundle non ci sono importi comparabili, scrivi "
    "'costo da stimare' invece di ancorarlo a un progetto inesistente; (5) chi "
    "sarebbe l'attuatore-tipo (Comune, consorzio ASI, Regione, partenariato "
    ...
    "- `fattibilita`: lo spend ratio OpenCoesione è SPECIFICO PER TEMA — mettilo "
    "in `spend_ratio_storico` SOLO per un'idea dello stesso tema (lo spend ratio "
    "'inclusione sociale' vale per un'idea welfare, NON per turismo, lavoro o "
    "commercio); per le idee di altro tema lascia `spend_ratio_storico: null`. "
    "Per il gap_comparativo riporta nel `dettaglio` gli importi reali dei progetti "
    ...
    "In particolare un progetto comparabile (titolo, CLP, importo, % di esito) "
    "è ammesso SOLO se presente nel bundle e citato col suo URL `/progetti/…`: "
...
MARKETING_INSTRUCTIONS = (
    ...
    '  "disclaimer": str\n'
    "}\n\n"
    "REGOLA (A) OBBLIGATORIA + (B) BONUS: ogni spunto DEVE citare nelle `evidenze` "
    "(A) una PREMESSA LOCALE verificabile (fonte ∈ "
    "istat/opencoesione/osm/ispra/ckan/kg — un asset, un flusso, un dato). "
    "(B) un PRECEDENTE ESTERNO con `fonte`=\"web\" e URL risolvibile (l'iniziativa "
    "altrui da cui prendi spunto) è FORTEMENTE CONSIGLIATO e alza la fattibilità — "
    "MA NON obbligatorio: se nella sezione WEB non trovi un precedente pertinente "
    "(es. fonte web assente), PRODUCI COMUNQUE lo spunto sulla sola premessa locale, "
    "con fattibilità più prudente (mai 'alta') e SENZA inventare un precedente. Uno "
    "spunto senza nemmeno la premessa locale (A) viene SCARTATO. Il precedente "
    "esterno è ISPIRAZIONE, MAI prova per questo comune: nel `dettaglio` scrivi "
    "'spunto da: …'.\n"
